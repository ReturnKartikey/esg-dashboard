...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'screenshot_2_ingest_hub_initial.png') });
    ...
    // Let's close the drawer if it's still open (sometimes clicking ledger row re-opens it or leaves it open)
    const drawerCloseBtn = page.locator('.drawer-close').first();
